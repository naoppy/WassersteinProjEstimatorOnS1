...
def run_once(i, true_mu, true_kappa, N: int) -> npt.NDArray[np.float64]:
    ...
    MLE_mu = MLE[0]
    MLE_CV = wrapedcauchy.circular_variance(MLE[1])
    ...
    W1method2_CV = wrapedcauchy.circular_variance(est.x[1])
    ...
    W1method3_CV = wrapedcauchy.circular_variance(est.x[1])
    ...
    W2method3_CV = wrapedcauchy.circular_variance(est.x[1])
...
def main():
    ...
    true_mu = 0.3
    true_kappa = 2
    true_CV = vonmises.circular_variance(true_kappa)

    print(f"true mu={true_mu}, true kappa={true_kappa}, true CV={true_CV}")
    print("(mu, CV, time)")
    ...
    for j, (N, try_num) in enumerate(
        ...
        MLE_CV_mse = np.mean((MLE_CV - true_kappa) ** 2)
