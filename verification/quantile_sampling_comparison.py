...
# Helper functions for new sampling
@numba.njit(fastmath=True, cache=True)
def _find_lefts_numba_old(x, z, step_grid):
    n_x = len(x)
    n_z = len(z)
    lefts = np.empty(n_x, dtype=np.float64)
    i = 0
...
def fast_quantile_sampling_grid_only(
    mu: float, kappa: float, sample_num: int
) -> np.ndarray:
    x, step = np.linspace(0, 1, sample_num, endpoint=False, retstep=True)
    x = x + step / 2

    # 1,048,576 points grid
    y0, step_grid = np.linspace(-np.pi, np.pi, 1048576, retstep=True)
    n_terms = 150
    n_arr = np.arange(1, n_terms + 1)
    r_n = ive(n_arr, kappa) / ive(0, kappa)

    # base for mu=0, theta=-np.pi is always -0.5, so F(y0) + 0.5
    z = vonmises._vonmises_cdf_series_numba_mu0(y0, r_n) + 0.5

    lefts = _find_lefts_numba_old(x, z, step_grid)
    samples = lefts + step_grid / 2

    # [0, 2*pi] mapping and topology correction (mu post-shifting)
...
def run_comparison():
    ...
    _ = fast_quantile_sampling_grid_only(mu, kappa, 10)
    ...
    for N in scales:
        ...
        t0 = time.perf_counter()
        samples_grid = fast_quantile_sampling_grid_only(mu, kappa, N)
        t_grid = time.perf_counter() - t0

        # Calculate error of grid method against scipy
        err_grid = np.max(
            np.minimum(
                np.abs(samples_grid - samples_scipy),
                2 * np.pi - np.abs(samples_grid - samples_scipy),
            )
        )
        print(
            f"{'quantile_sampling (SciPy)':<30} | {N:<6} | {t_scipy:<10.4f} | "
            f"{'0.00e+00 (Reference)':<18}"
        )
        print(
            f"{'fast_quantile_sampling (Grid)':<30} | {N:<6} | {t_grid:<10.4f} | "
            f"{err_grid:<18.2e}"
        )
        ...
        err_newton1 = np.max(
            np.minimum(
                np.abs(samples_newton1 - samples_scipy),
                2 * np.pi - np.abs(samples_newton1 - samples_scipy),
            )
        )
        print(
            f"{'1-step Newton (Coarse Grid)':<30} | {N:<6} | {t_newton1:<10.4f} | "
            f"{err_newton1:<18.2e}"
        )
        ...
        err_newton2 = np.max(
            np.minimum(
                np.abs(samples_newton2 - samples_scipy),
                2 * np.pi - np.abs(samples_newton2 - samples_scipy),
            )
        )
        print(
            f"{'2-step Newton (Coarse Grid)':<30} | {N:<6} | {t_newton2:<10.4f} | "
            f"{err_newton2:<18.2e}"
        )
        print("-" * 75)
