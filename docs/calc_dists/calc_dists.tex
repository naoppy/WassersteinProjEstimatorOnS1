...
Now, assume one distribution (say $P$) is more concentrated than the other ($Q$). For symmetric, unimodal distributions centered at $0$, the probability density functions cross exactly once in $(0, \pi)$, meaning the difference $F_P(\theta) - F_Q(\theta)$ does not change sign on $(0, \pi)$. Specifically, if $P$ is more concentrated than $Q$, we have $F_P(\theta) \geq F_Q(\theta)$ for all $\theta \in [0, \pi]$. This allows us to remove the absolute value:
\begin{equation}
W_1(P, Q) = 2 \int_0^\pi (F_P(\theta) - F_Q(\theta)) d\theta
\end{equation}
